...
func commandCatch(c *config, args ...string) error {
	...
	if len(args) < 1 {
		return fmt.Errorf("Give the name of the pokemon you want to catch")
	}

	url := fmt.Sprintf("https://pokeapi.co/api/v2/pokemon/%s", args[0])
	res, err := http.Get(url)
	if err != nil {
		return fmt.Errorf("Could not get Pokemon from API %w", err)
	}
	...
	if err := decoder.Decode(&pokemon); err != nil {
		return fmt.Errorf("Could not decode Server Res to Go Struct")
	}
...
// will add pokemon to users pokemon collection
